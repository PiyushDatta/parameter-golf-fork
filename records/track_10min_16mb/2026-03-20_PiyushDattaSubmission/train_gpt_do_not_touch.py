...
chmod +x /data/repos/parameter-golf-fork/.venv/lib/python3.12/site-packages/triton/backends/nvidia/bin/ptxas

with-proxy DATA_PATH=./data/datasets/fineweb10B_sp8192 TOKENIZER_PATH=./data/tokenizers/fineweb_8192_bpe.model VOCAB_SIZE=8192 with-proxy torchrun --nproc_per_node=2 records/track_10min_16mb/2026-03-20_PiyushDattaSubmission/train_gpt_do_not_touch.py
"""

import collections, copy, glob, io, lzma, math, os
from pathlib import Path
import random, sys, time, uuid, numpy as np, sentencepiece as spm, torch, torch.distributed as dist, torch.nn.functional as F
...
class Hyperparameters:
    ...
    iterations = 20000
    warmdown_frac = float(os.environ.get("WARMDOWN_FRAC", 0.55))
    ...
    swa_start_frac = float(os.environ.get("SWA_START_FRAC", 0.1))
    swa_every = int(os.environ.get("SWA_EVERY", 2))
...
class ValidationData:
    def __init__(self, h, device):
        ...
        if int(self.sp.vocab_size()) != h.vocab_size:
            raise ValueError(f"vocab mismatch")
...
def build_sentencepiece_luts(sp, vocab_size, device):
    ...
    assert sp.piece_to_id("▁") != sp.unk_id(), "bad tokenizer"
...
def load_validation_tokens(pattern, seq_len):
    ...
    if usable <= 0:
        raise ValueError(f"val too short")
...
class ShuffledSequenceLoader:
    def __init__(self, h, device):
        ...
        if not all_files:
            raise FileNotFoundError(f"no files")
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim, num_heads, num_kv_heads, rope_base, qk_gain_init, train_seq_len):
        super().__init__()
        if dim % num_heads != 0:
            raise ValueError("dim%heads!=0")
        if num_heads % num_kv_heads != 0:
            raise ValueError("heads%kv!=0")
        self.num_heads = num_heads
        self.num_kv_heads = num_kv_heads
        self.head_dim = dim // num_heads
        if self.head_dim % 2 != 0:
            raise ValueError("head_dim odd")
    ...
    def forward(self, x):
        ...
        if _HAS_FA3:
            ...
            if rep > 1:
                k = (
                    k[:, :, :, None, :]
                    .expand(bsz, seqlen, self.num_kv_heads, rep, self.head_dim)
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        if h.logit_softcap <= 0.0:
            raise ValueError(f"bad softcap")
    ...
    def forward(self, input_ids, target_ids):
        ...
        z_coeff = float(os.environ.get("Z_LOSS_COEFF", "0"))
        if z_coeff > 0:
            ce = ce + z_coeff * torch.logsumexp(logits_flat, dim=-1).square().mean()
        return ce
...
def gptq_quantize_weight(w, H, clip_sigmas=3.0, clip_range=63, block_size=128):
    ...
    dead = torch.diag(H) == 0
    H[dead, dead] = 1
    damp = float(os.environ.get("GPTQ_DAMP_FRAC", ".01")) * H.diag().mean()
...
def gptq_mixed_quantize(state_dict, hessians, h):
    ...
    for name, cat in meta.items():
        categories[cat].add(name.split(".")[0])
...
def serialize(h, base_model, code):
    ...
    if len(quant_blob) > target_bytes:
        ...
        for name, info in quant_meta.items():
            ...
            s = quant_result[name + ".scale"]
            sf = s.float().view(-1) if s.ndim == 1 else s.float()[:, 0]
            for r in range(q.shape[0]):
                for c in range(q.shape[1]):
                    v = int(q[r, c])
                    if 0 < abs(v) <= 2:
                        candidates.append((float(sf[r]) ** 2 * v * v, name, r, c))
        candidates.sort()
        n_zero = min(len(candidates), int(over * 20))
        log(f"prune:{len(candidates)} cand, zeroing {n_zero}")
        for _, name, r, c in candidates[:n_zero]:
            quant_result[name + ".q"][r, c] = 0
        quant_buf = io.BytesIO()
        torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
        quant_raw = quant_buf.getvalue()
        quant_blob = _compress(quant_raw, h.compressor)
...
def eval_val(h, device, val_data, model):
    ...
    if local_batch_tokens < seq_len:
        raise ValueError(f"batch too small")
...
def train_model(h, device, val_data):
    ...
    if h.swa_enabled and swa_state is not None and swa_count > 0:
        log(f"swa:applying SWA weights ({swa_count} checkpoints)")
...
def main():
    ...
    if world_size <= 0:
        raise ValueError(f"bad ws")
    if 8 % world_size != 0:
        raise ValueError(f"ws must divide 8")
